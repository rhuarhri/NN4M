...
class OnlineDatabase () {
    ...
    fun getAvailableClothes(): ArrayList<SearchItem> {
        val inStream: InputStream = getInputStream(CLOTHING_REGISTER_URL)
    ...
    private fun getClothingList(reader: JsonReader): ArrayList<SearchItem> {
        ...
        val clothingList: ArrayList<SearchItem> = ArrayList()
    ...
    private fun getInputStream(jsonURL: String): InputStream {
        ...
        newInputStream = connection.inputStream

        return newInputStream!!
    }

    @Throws(IOException::class)
    fun isStreamEmpty(inStream: InputStream): Boolean {
    ...
    @Throws (Exception::class)
    fun getItemDescription(descriptionURL : String) : ItemDescription
    {
        val returnedItemDescription = ItemDescription()

        val userLocation = getLocation()

        val inStream: InputStream = getInputStream(descriptionURL)
            ...
            when (userLocation) {
                "UK" -> {
                    ...
                    else if (name == "wascost")
                    {
                        ...
                        val fromFile = reader.nextString()
                        if (fromFile == "")
                        {
                            wasCost = cost
                        }
